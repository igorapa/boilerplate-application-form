...
import { Box, Padding, Flexbox, FadeIn } from '@/ui';
...
export default function FormContainer(props): JSX.Element {
  ...
  React.useEffect(() => {
    const newDraft = {
      ...draft,
      ...fields.reduce((prev, next) => ({ ...prev, [next.id]: formData[next.id] || next.defaultValue || '' }), {}),
    };
    setDraft(newDraft);
    setErrors(
      fields.reduce((prev, next) => {
        ...
        if (next.dependsOn) {
          error = next.dependsOn.some((item) => newDraft[item])
            ? getTextError(formData[next.id] || '', next.isRequired)
            : '';
        }
        return {
          ...prev,
  ...
  const getCurrentValid = () => {
    ...
    const newErrors = errorsList.reduce(
      (prev, next) => ({ ...prev, [next.id]: { ...errors[next.id], isInvalid: true } }),
      {},
    );
  ...
  return (
    <Padding top={80}>
      <Flexbox alignItems="center" justifyContent="center">
        <FadeIn duration="500ms" yOffset="30px">
          <Box border={1} borderRadius={10} width={640} backgroundColor="white">
